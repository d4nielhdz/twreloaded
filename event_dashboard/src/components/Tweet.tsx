...
const Tweet = ({ tweet }: { tweet: RenderedTweet }) => {
  ...
  const getLikesButtonContent = () => {
    if (like) {
      return (
        <>
          <AiFillHeart className="red" /> {likes + 1}
        </>
      );
    } else {
      return (
  ...
  return (
    <div className="tweet" onClick={seeTweet}>
      {tweet.replyTo && (
        <button className="btn link reply" onClick={seeReply}>
          <FaReplyAll /> Respuesta
        </button>
      )}
      <div className="tweet-user">
        <button className="bold btn link" onClick={seeProfile}>
          {tweet.user.username}
        </button>
      </div>
