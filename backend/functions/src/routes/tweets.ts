...
router.get("/followers", verifyToken, async (req, res) => {
  ...
  const userSnapshot = await db.collection("users").doc(authUserId).get();
  const userData = userSnapshot.data()!;
  const followedUsersIds = userData.followedUsers ?? [];
  ...
  const followedUsersSnapshot = await db
    .collection("users")
    .where(admin.firestore.FieldPath.documentId(), "in", followedUsersIds)
    .get();
  ...
  snapshot.forEach((tweet) => {
    ...
    const user = followedUsers.find((u) => u.id == data.userId);
...
router.get("/:id", async (req, res) => {
  ...
  const tweetUserSnapshot = await db
    .collection("users")
